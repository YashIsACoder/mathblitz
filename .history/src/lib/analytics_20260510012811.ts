...
export async function getOperationSwitchingCost(userId: string, startDate?: string | null, endDate?: string | null) {
  ...
  const switchingCosts: Record<string, { count: number; avgLatency: number; baselineLatency: number; cost: number }> = {};
  const baselineLatencies: Record<string, { sum: number; count: number }> = {};

  // Calculate baseline latencies for each operation (when not switching)
  for (let i = 1; i < attempts.length; i++) {
    const currentOp = attempts[i].operation;
    const prevOp = attempts[i - 1].operation;
    
    if (currentOp === prevOp) {
      if (!baselineLatencies[currentOp]) {
        baselineLatencies[currentOp] = { sum: 0, count: 0 };
      }
      baselineLatencies[currentOp].sum += attempts[i].latencyMs;
      baselineLatencies[currentOp].count++;
    }
  }

  const baselines: Record<string, number> = {};
  for (const [op, data] of Object.entries(baselineLatencies)) {
    baselines[op] = data.sum / data.count;
  }
  ...
  for (let i = 1; i < attempts.length; i++) {
    ...
    if (currentOp !== prevOp) {
      const transition = `${prevOp}→${currentOp}`;
      if (!switchingCosts[transition]) {
        switchingCosts[transition] = { count: 0, avgLatency: 0, baselineLatency: baselines[currentOp] || 0, cost: 0 };
      }
      switchingCosts[transition].count++;
      switchingCosts[transition].avgLatency = (switchingCosts[transition].avgLatency * (switchingCosts[transition].count - 1) + attempts[i].latencyMs) / switchingCosts[transition].count;
    }
  }

  // Calculate cost as percentage increase over baseline
  for (const [transition, data] of Object.entries(switchingCosts)) {
    if (data.baselineLatency > 0) {
      data.cost = ((data.avgLatency - data.baselineLatency) / data.baselineLatency) * 100;
    }
  }

  // Generate insights
  const insights: string[] = [];
  const highCostTransitions = Object.entries(switchingCosts)
    .filter(([_, data]) => data.cost > 20 && data.count >= 3)
    .sort((a, b) => b[1].cost - a[1].cost)
    .slice(0, 3);
  
  if (highCostTransitions.length > 0) {
    insights.push(`High switching costs detected: ${highCostTransitions.map(([t, d]) => `${t} (+${d.cost.toFixed(0)}%)`).join(', ')}`);
  }
  
  const avgCost = Object.values(switchingCosts).reduce((sum, d) => sum + d.cost, 0) / Object.values(switchingCosts).length || 0;
  if (avgCost > 10) {
    insights.push(`Average operation switching cost is ${avgCost.toFixed(0)}% - consider practicing operation transitions`);
  } else if (avgCost < 5) {
    insights.push('Low operation switching cost - you handle operation changes well');
  }
  
  if (insights.length === 0) {
    insights.push('No significant operation switching costs detected');
  }
