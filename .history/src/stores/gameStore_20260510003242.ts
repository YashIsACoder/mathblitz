...
export const useGameStore = create<GameState>()(
  subscribeWithSelector((set, get) => ({
    ...
    submitAnswer: (typed: string) => {
      ...
      const carryCount = calculateCarryCount(state.currentQuestion.lhs, state.currentQuestion.rhs, state.currentQuestion.operation);
      const borrowCount = calculateBorrowCount(state.currentQuestion.lhs, state.currentQuestion.rhs, state.currentQuestion.operation);
      const difficultyScore = calculateDifficultyScore(state.currentQuestion.lhs, state.currentQuestion.rhs, state.currentQuestion.operation);
      const cognitiveTags = generateCognitiveTags(state.currentQuestion.lhs, state.currentQuestion.rhs, state.currentQuestion.operation);
      const sessionElapsedMs = state.startTime ? now - state.startTime : 0;
      const streakBefore = state.correctAttempts;
