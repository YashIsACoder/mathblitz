...
export const useGameStore = create<GameState>()(
  subscribeWithSelector((set, get) => ({
    ...
    startGame: async () => {
      ...
      // Create session in database and get the real sessionId
      let sessionId;
      try {
    ...
    submitAnswer: (typed: string) => {
      ...
      // Calculate potential score after this answer
      const potentialScore = correct ? state.score + 1 : state.score;
      const potentialTotalAttempts = state.totalAttempts + 1;
      const potentialAccuracy = potentialScore / potentialTotalAttempts;

      // Filter out outlier attempts (sessions with 0-3 score or very low accuracy)
      const shouldLogAttempt = potentialScore > 3 || (potentialTotalAttempts >= 10 && potentialAccuracy > 0.1);
      ...
      set(s => ({
        ...
        pendingAttempts: shouldLogAttempt ? [...s.pendingAttempts, attempt] : s.pendingAttempts,
      }));
